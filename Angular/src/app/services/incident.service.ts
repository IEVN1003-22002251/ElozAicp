...
export interface Incident {
  id?: string; incident_type: string; description?: string; location?: string;
  severity: 'low' | 'medium' | 'high' | 'critical';
  status: 'reported' | 'in_progress' | 'resolved' | 'closed';
  reported_by?: string; reported_at?: string; resolved_at?: string; resolution_notes?: string;
  created_at?: string; updated_at?: string; fraccionamiento_id?: string; [key: string]: any;
}
...
export class IncidentService {
  ...
  getIncidents(params?: { status?: string; incident_type?: string; severity?: string; fraccionamiento_id?: string; start_date?: string; end_date?: string }): Observable<{ success: boolean; data: Incident[] }> {
